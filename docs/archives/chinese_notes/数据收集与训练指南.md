# 数据收集与训练完整指南

**项目**: 3D打印机轨迹误差预测与修正
**更新日期**: 2026-01-31
**目的**: 使用固件增强仿真数据训练误差修复神经网络

---

## 📋 目录

1. [快速开始](#快速开始)
2. [数据收集](#数据收集)
3. [数据准备](#数据准备)
4. [模型训练](#模型训练)
5. [模型评估](#模型评估)
6. [常见问题](#常见问题)

---

## 🚀 快速开始

### 最小化流程（推荐新手）

```bash
# 1. 收集单个文件的采样数据（5层，约10分钟）
matlab -batch "collect_3dbenchy('sampled:5')"

# 2. 准备训练数据
python data/scripts/prepare_training_data.py

# 3. 训练模型
python experiments/train_trajectory_model.py --epochs 50

# 4. 评估模型
python experiments/evaluate_trajectory_model.py
```

---

## 📊 数据收集

### 步骤1: MATLAB仿真

#### 方式1: 使用便捷脚本（推荐）

项目已为每个gcode文件创建了便捷脚本：

```matlab
% 收集3DBenchy的采样数据（每5层采样1层）
collect_3dbenchy('sampled:5');

% 收集Bearing的采样数据
collect_bearing5('sampled:5');

% 收集Nautilus的采样数据
collect_nautilus('sampled:5');

% 收集Boat的采样数据
collect_boat('sampled:5');
```

#### 方式2: 批量收集所有文件

```matlab
% 收集所有文件，每5层采样1层
collect_all  % 这个脚本会自动处理所有gcode文件
```

#### 方式3: 使用核心函数（灵活）

```matlab
% 单个文件，指定层范围
collect_data('test_gcode_files/3DBenchy_PLA_1h28m.gcode', 1:50, ...
             'GPU', 1, ...
             'UseFirmwareEffects', true);  % 使用固件增强仿真

% 多个文件，不同配置
files = {'file1.gcode', 'file2.gcode'};
configs = {'sampled:5', 'all'};
collect_data(files, configs, 'UseFirmwareEffects', true);
```

### 层数配置说明

| 配置 | 含义 | 示例 | 用途 |
|------|------|------|------|
| `'all'` | 所有层 | 1:48 | 完整数据（时间长） |
| `'sampled:N'` | 每N层采样1层 | sampled:5 | 快速测试（推荐） |
| `start:end` | 指定范围 | 1:20 | 部分数据 |
| 单层 | 单个层 | 25 | 调试 |

### 推荐的数据收集策略

**策略1: 快速验证**（~30分钟）
```matlab
% 每个文件采样5层
collect_3dbenchy('sampled:10');  % 约5层
collect_bearing5('sampled:10');
collect_nautilus('sampled:10');
% 总计: ~15层 × 3文件 × 2分钟/层 = ~90分钟
```

**策略2: 标准训练**（~2小时）
```matlab
% 每个文件采样5层
collect_3dbenchy('sampled:5');   % 约10层
collect_bearing5('sampled:5');   % 约10层
collect_nautilus('sampled:5');   % 约10层
collect_boat('sampled:5');       % 约10层
% 总计: ~40层 × 2分钟/层 = ~80分钟
```

**策略3: 完整数据**（~6小时）
```matlab
% 所有文件的所有层
collect_all
% 总计: ~200层 × 2分钟/层 = ~400分钟
```

### 查看现有数据

```bash
# 检查已收集的数据
ls -lh data_simulation_*/

# 统计数据点
python check_data_stats.py
```

### 固件增强仿真参数

当前配置（已在`collect_data.m`中设置）：

| 参数 | 值 | 说明 |
|------|-----|------|
| `UseFirmwareEffects` | `true` | 启用固件增强（0.1mm误差） |
| `GPU` | `1` | 使用GPU加速 |
| `Resume` | `true` | 跳过已完成的层 |

**验证误差范围**:

```matlab
% 测试固件增强仿真
test_firmware_effects_simple

% 预期输出:
%   RMS误差: ~140 μm ✅
%   最大误差: 80-150 μm ✅
```

---

## 🔄 数据准备

### 步骤2: 生成训练数据集

MATLAB仿真生成的`.mat`文件需要转换为PyTorch可用的格式。

#### 运行数据准备脚本

```bash
# 基础版本（推荐）
python data/scripts/prepare_training_data.py \
    --data_dirs data_simulation_* \
    --output_dir data/processed \
    --sequence_length 128 \
    --stride 4
```

#### 参数说明

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--data_dirs` | `data_simulation_*` | 输入数据目录（支持通配符） |
| `--output_dir` | `data/processed` | 输出目录 |
| `--sequence_length` | `128` | 序列长度（时间步） |
| `--stride` | `4` | 采样步长（越小数据越多） |
| `--train_ratio` | `0.8` | 训练集比例 |
| `--val_ratio` | `0.1` | 验证集比例 |
| `--test_ratio` | `0.1` | 测试集比例 |

#### 数据增强选项

```bash
# 添加噪声增强
python data/scripts/prepare_training_data.py \
    --augment_noise \
    --noise_level 0.01

# 添加时序偏移增强
python data/scripts/prepare_training_data.py \
    --augment_temporal \
    --temporal_shift 5
```

#### 数据集统计

查看生成的数据集：

```bash
# 查看数据集信息
python -c "
from data.simulation.dataset import PrinterSimulationDataset
dataset = PrinterSimulationDataset('data/processed/train')
print(f'样本数: {len(dataset)}')
print(f'特征维度: {dataset[0][\"input_features\"].shape}')
print(f'目标维度: {dataset[0][\"trajectory_targets\"].shape}')
"
```

**预期输出**:
```
样本数: ~50000
特征维度: torch.Size([128, 15])  # 128时间步, 15特征
目标维度: torch.Size([128, 2])   # 128时间步, 2轴误差
```

---

## 🧠 模型训练

### 步骤3: 配置训练参数

#### 创建配置文件

在`config/`目录创建自定义配置：

```python
# config/my_training_config.py
from config.base_config import BaseConfig

class MyConfig(BaseConfig):
    # 数据配置
    data:
        root_dir: 'data/processed'
        batch_size: 32
        num_workers: 4
        sequence_length: 128
        stride: 4

    # 模型配置
    model:
        type: 'TrajectoryErrorTransformer'
        d_model: 256
        nhead: 8
        num_layers: 6
        dim_feedforward: 1024
        dropout: 0.1

    # 训练配置
    training:
        epochs: 100
        learning_rate: 0.0001
        weight_decay: 0.0001
        early_stopping_patience: 15
        mixed_precision: true
        gradient_clip: 1.0

    # 设备配置
    device: 'cuda'  # 或 'cpu'
    experiment_name: 'trajectory_error_correction'
    log_dir: 'logs/run1'
```

### 步骤4: 开始训练

#### 基础训练命令

```bash
# 使用默认配置训练
python experiments/train_trajectory_model.py

# 指定配置文件
python experiments/train_trajectory_model.py \
    --config config/my_training_config.py

# 覆盖特定参数
python experiments/train_trajectory_model.py \
    --epochs 100 \
    --batch_size 64 \
    --lr 0.0001 \
    --gpu 0
```

#### 完整训练示例

```bash
# 训练100个epoch，批次大小32，学习率0.0001
python experiments/train_trajectory_model.py \
    --data_root data/processed \
    --batch_size 32 \
    --epochs 100 \
    --lr 0.0001 \
    --device cuda:0 \
    --experiment_name firmware_enhanced_v1 \
    --log_dir logs/firmware_v1 \
    --save_dir checkpoints/firmware_v1 \
    --mixed_precision \
    --early_stopping_patience 15
```

#### 训练参数详解

| 参数 | 推荐值 | 说明 |
|------|--------|------|
| `--epochs` | 50-100 | 训练轮数 |
| `--batch_size` | 32-64 | 批次大小（GPU内存允许越大越好） |
| `--lr` | 0.0001 | 学习率 |
| `--device` | `cuda:0` | 设备（`cuda:0`, `cuda:1`, `cpu`） |
| `--mixed_precision` | - | 启用混合精度训练（更快） |
| `--gradient_clip` | 1.0 | 梯度裁剪（防止梯度爆炸） |
| `--early_stopping_patience` | 15 | 早停耐心值 |

#### 监控训练进度

训练时会实时显示：

```
Epoch 1/100
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ 100%
    Batch   20/156 | Loss: 0.023456 | X-Err: 0.012345 | Y-Err: 0.011234 | Throughput:   512 samples/s
    Batch   40/156 | Loss: 0.021234 | X-Err: 0.011234 | Y-Err: 0.010123 | Throughput:   508 samples/s
    ...
  ▶ Train Loss: 0.019876 | X-Err: 0.010234 | Y-Err: 0.009876
  ▶ Val Loss: 0.018765 | X-Err: 0.009876 | Y-Err: 0.009234
  ✓ Saved checkpoint: checkpoints/firmware_v1/best_model.pt
```

#### 使用TensorBoard监控

```bash
# 启动TensorBoard
tensorboard --logdir logs/firmware_v1 --port 6006

# 浏览器打开
# http://localhost:6006
```

**监控指标**:
- 损失曲线（Train/Val Loss）
- X/Y轴误差（X-Err/Y-Err）
- 学习率变化
- 梯度分布

---

## 📈 模型评估

### 步骤5: 评估训练好的模型

#### 基础评估

```bash
# 评估最佳模型
python experiments/evaluate_trajectory_model.py \
    --checkpoint checkpoints/firmware_v1/best_model.pt \
    --data_root data/processed/test
```

#### 详细评估

```bash
# 完整评估报告
python experiments/evaluate_trajectory_model.py \
    --checkpoint checkpoints/firmware_v1/best_model.pt \
    --data_root data/processed/test \
    --output_dir results/evaluation \
    --visualize \
    --save_predictions
```

#### 评估指标

**关键指标**:

| 指标 | 目标值 | 说明 |
|------|--------|------|
| **MAE (X轴)** | < 0.02 mm | 平均绝对误差 |
| **MAE (Y轴)** | < 0.02 mm | 平均绝对误差 |
| **RMSE (X轴)** | < 0.03 mm | 均方根误差 |
| **RMSE (Y轴)** | < 0.03 mm | 均方根误差 |
| **R² (X轴)** | > 0.85 | 决定系数 |
| **R² (Y轴)** | > 0.85 | 决定系数 |

**示例输出**:
```
===========================================
Trajectory Error Model Evaluation
===========================================
Dataset: Test (5234 samples)

Overall Metrics:
  MAE:  0.0156 mm
  RMSE: 0.0223 mm
  R²:   0.8923

Per-Axis Metrics:
  X-Axis:
    MAE:  0.0145 mm
    RMSE: 0.0201 mm
    R²:   0.9012

  Y-Axis:
    MAE:  0.0167 mm
    RMSE: 0.0245 mm
    R²:   0.8834

Percentile Errors:
  50th (Median): 0.0123 mm
  90th:         0.0345 mm
  95th:         0.0456 mm
  99th:         0.0678 mm

✓ Model meets performance targets!
===========================================
```

### 可视化结果

#### 生成可视化图表

```python
# 使用可视化脚本
python experiments/visualize_trajectory_correction.py \
    --checkpoint checkpoints/firmware_v1/best_model.pt \
    --data_dir data/processed/test \
    --num_samples 10 \
    --output_dir results/visualizations
```

**生成的图表**:
1. `trajectory_comparison.png` - 名义轨迹 vs 修正轨迹
2. `error_distribution.png` - 误差分布直方图
3. `error_correlation.png` - 预测误差 vs 真实误差散点图
4. `per_axis_error.png` - X/Y轴误差对比
5. `sequence_prediction.png` - 序列预测示例

#### 交互式可视化

```python
# Jupyter Notebook中使用
from models.trajectory import TrajectoryErrorTransformer
from data.simulation.dataset import PrinterSimulationDataset
import torch

# 加载模型
model = TrajectoryErrorTransformer.load_from_checkpoint(
    'checkpoints/firmware_v1/best_model.pt'
)

# 加载数据
dataset = PrinterSimulationDataset('data/processed/test')
sample = dataset[0]

# 预测
with torch.no_grad():
    predictions = model(sample['input_features'].unsqueeze(0))

# 可视化
import matplotlib.pyplot as plt

plt.figure(figsize=(15, 5))

# 子图1: 名义轨迹
plt.subplot(1, 3, 1)
plt.plot(sample['trajectory_targets'][:, 0], label='True X')
plt.plot(predictions['error_x'][0, :, 0], label='Pred X', linestyle='--')
plt.title('X-Axis Error')
plt.legend()

# 子图2: Y轴误差
plt.subplot(1, 3, 2)
plt.plot(sample['trajectory_targets'][:, 1], label='True Y')
plt.plot(predictions['error_y'][0, :, 0], label='Pred Y', linestyle='--')
plt.title('Y-Axis Error')
plt.legend()

# 子图3: 误差幅值
plt.subplot(1, 3, 3)
true_mag = torch.sqrt(sample['trajectory_targets'][:, 0]**2 +
                      sample['trajectory_targets'][:, 1]**2)
pred_mag = torch.sqrt(predictions['error_x'][0, :, 0]**2 +
                      predictions['error_y'][0, :, 0]**2)
plt.plot(true_mag, label='True')
plt.plot(pred_mag, label='Pred', linestyle='--')
plt.title('Error Magnitude')
plt.legend()

plt.tight_layout()
plt.savefig('prediction_visualization.png', dpi=150)
plt.show()
```

---

## 🔧 常见问题

### Q1: MATLAB仿真太慢怎么办？

**A**: 优化策略：

1. **减少采样层数**:
   ```matlab
   collect_3dbenchy('sampled:20');  % 每20层采样1层（而非5层）
   ```

2. **使用GPU加速**（已默认启用）:
   ```matlab
   collect_data(..., 'GPU', 1);
   ```

3. **并行处理**:
   ```matlab
   % 开启多个MATLAB实例，同时收集不同文件
   % 实例1:
   collect_3dbenchy('sampled:5');
   % 实例2:
   collect_bearing5('sampled:5');
   ```

4. **减少序列长度**:
   ```matlab
   % 在reconstruct_trajectory中修改time_step
   trajectory_options.time_step = 0.02;  % 从0.01改为0.02
   ```

### Q2: 训练时显存不足（CUDA OOM）

**A**: 解决方案：

1. **减小批次大小**:
   ```bash
   python train_trajectory_model.py --batch_size 16  # 从32减到16
   ```

2. **减小序列长度**:
   ```bash
   python prepare_training_data.py --sequence_length 64  # 从128减到64
   ```

3. **启用梯度累积**:
   ```python
   # 修改训练脚本
   accumulation_steps = 4
   if (batch_idx + 1) % accumulation_steps == 0:
       optimizer.step()
       optimizer.zero_grad()
   ```

4. **使用混合精度训练**（已默认启用）:
   ```bash
   python train_trajectory_model.py --mixed_precision
   ```

### Q3: 模型不收敛（Loss不下降）

**A**: 排查步骤：

1. **检查数据质量**:
   ```python
   python check_data_stats.py
   # 确认误差范围在0.1mm左右
   ```

2. **调整学习率**:
   ```bash
   # 尝试更小的学习率
   python train_trajectory_model.py --lr 0.00001
   ```

3. **使用学习率调度器**（已内置）:
   ```python
   # 配置文件中已配置ReduceLROnPlateau
   scheduler:
       type: 'reduce_on_plateau'
       factor: 0.5
       patience: 5
   ```

4. **增加模型容量**:
   ```python
   # 增加Transformer层数或隐藏维度
   model:
       num_layers: 8  # 从6增加到8
       d_model: 512   # 从256增加到512
   ```

### Q4: 误差预测不准确

**A**: 改进策略：

1. **增加数据量**:
   ```matlab
   % 收集更多层数
   collect_3dbenchy('sampled:3');  # 从5改为3（更多层）
   ```

2. **添加数据增强**:
   ```bash
   python prepare_training_data.py --augment_noise --noise_level 0.01
   ```

3. **调整序列长度**:
   ```bash
   # 更长的序列可能捕捉更多上下文
   python prepare_training_data.py --sequence_length 256
   ```

4. **尝试不同的模型架构**:
   ```python
   # 修改Transformer的头数或层数
   model:
       nhead: 16  # 从8增加到16
       num_layers: 8
   ```

### Q5: 如何恢复中断的训练？

**A**: 从检查点恢复：

```bash
# 训练脚本会自动保存检查点
# checkpoints/firmware_v1/last_model.pt
# checkpoints/firmware_v1/best_model.pt

# 从检查点恢复训练
python train_trajectory_model.py \
    --resume checkpoints/firmware_v1/last_model.pt \
    --epochs 200  # 继续训练更多轮
```

### Q6: 如何在新数据上测试？

**A**: 推理流程：

```python
from models.trajectory import TrajectoryErrorTransformer
import torch

# 加载模型
model = TrajectoryErrorTransformer.load_from_checkpoint(
    'checkpoints/firmware_v1/best_model.pt'
)
model.eval()

# 准备输入数据
# input_features: [batch_size, sequence_length, num_features]
input_features = prepare_your_data()

# 预测
with torch.no_grad():
    predictions = model(input_features)

# 提取误差
error_x = predictions['error_x']  # [batch_size, sequence_length, 1]
error_y = predictions['error_y']  # [batch_size, sequence_length, 1]

# 应用修正
corrected_x = nominal_x - error_x
corrected_y = nominal_y - error_y
```

### Q7: 如何对比不同模型？

**A**: 基准测试：

```bash
# 训练多个模型
python train_trajectory_model.py --exp_name model_v1 --lr 0.0001
python train_trajectory_model.py --exp_name model_v2 --lr 0.00005
python train_trajectory_model.py --exp_name model_v3 --lr 0.0002

# 对比评估
python experiments/evaluate_trajectory_model.py \
    --checkpoint checkpoints/model_v1/best_model.pt \
    --output_dir results/model_v1

python experiments/evaluate_trajectory_model.py \
    --checkpoint checkpoints/model_v2/best_model.pt \
    --output_dir results/model_v2

python experiments/evaluate_trajectory_model.py \
    --checkpoint checkpoints/model_v3/best_model.pt \
    --output_dir results/model_v3

# 生成对比报告
python -c "
import pandas as pd
import json

results = {}
for name in ['model_v1', 'model_v2', 'model_v3']:
    with open(f'results/{name}/metrics.json') as f:
        results[name] = json.load(f)

df = pd.DataFrame(results).T
print(df[['mae', 'rmse', 'r2']])
"
```

---

## 📚 附录

### A. 完整工作流程示例

```bash
# ========== 第1天：数据收集 ==========
# MATLAB命令窗口
matlab

% 在MATLAB中执行：
collect_3dbenchy('sampled:5');
collect_bearing5('sampled:5');
collect_nautilus('sampled:5');

exit  % 退出MATLAB

# ========== 第1天（晚上）：数据准备 ==========
# Bash/PowerShell
cd F:\TJ\3d_print\3d_printer_pinn4ieee

python data/scripts/prepare_training_data.py \
    --data_dirs data_simulation_* \
    --output_dir data/processed \
    --sequence_length 128 \
    --stride 4

# ========== 第2天：模型训练 ==========
# 训练模型（约2-4小时）
python experiments/train_trajectory_model.py \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.0001 \
    --experiment_name firmware_enhanced_v1 \
    --mixed_precision

# ========== 第2天（下午）：模型评估 ==========
# 评估模型
python experiments/evaluate_trajectory_model.py \
    --checkpoint checkpoints/firmware_enhanced_v1/best_model.pt \
    --visualize \
    --save_predictions

# ========== 第3天：结果分析 ==========
# 查看结果
python -c "
import json
with open('results/evaluation/metrics.json') as f:
    metrics = json.load(f)
print(json.dumps(metrics, indent=2))
"

# 生成可视化
python experiments/visualize_trajectory_correction.py \
    --checkpoint checkpoints/firmware_enhanced_v1/best_model.pt \
    --num_samples 20
```

### B. 数据需求估算

| 模型规模 | 参数量 | 推荐数据量 | 训练时间 |
|---------|--------|-----------|---------|
| 小型 | 1-2M | 50K-100K 样本 | 1-2小时 |
| 中型（当前） | 5M | 200K-500K 样本 | 2-4小时 |
| 大型 | 10M+ | 1M+ 样本 | 6-12小时 |

**计算公式**:
```
数据量 (样本) = 层数 × 点/层 / stride × sequence_length
```

**示例**:
```
40层 × 2000点/层 / 4 stride × 128 seq_len = 256,000 样本
```

### C. 硬件需求

**最低配置**:
- CPU: 4核
- RAM: 8 GB
- GPU: 无（CPU训练，慢10-20倍）
- 存储: 10 GB

**推荐配置**:
- CPU: 8核
- RAM: 16 GB
- GPU: GTX 1080 或更好（8 GB VRAM）
- 存储: 20 GB SSD

**理想配置**:
- CPU: 16核
- RAM: 32 GB
- GPU: RTX 3080 或更好（10+ GB VRAM）
- 存储: 50 GB NVMe SSD

### D. 训练时间估算

| 配置 | 批次大小 | 每epoch时间 | 总时间（100 epochs） |
|------|---------|------------|-------------------|
| CPU (i7-8700K) | 16 | ~15分钟 | ~25小时 |
| GPU (GTX 1080) | 32 | ~3分钟 | ~5小时 |
| GPU (RTX 3080) | 64 | ~1分钟 | ~2小时 |

**注**: 实际时间取决于数据量、序列长度、模型大小。

---

## 🎯 快速参考卡片

### 数据收集
```matlab
% 快速测试
collect_3dbenchy('sampled:10');  % ~5层

% 标准训练
collect_3dbenchy('sampled:5');   % ~10层

% 完整数据
collect_3dbenchy('all');         % ~48层
```

### 数据准备
```bash
python data/scripts/prepare_training_data.py \
    --data_dirs data_simulation_* \
    --output_dir data/processed
```

### 模型训练
```bash
python experiments/train_trajectory_model.py \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.0001
```

### 模型评估
```bash
python experiments/evaluate_trajectory_model.py \
    --checkpoint checkpoints/firmware_enhanced_v1/best_model.pt
```

---

**文档版本**: 1.0
**最后更新**: 2026-01-31
**作者**: 3D Printer PINN Project
