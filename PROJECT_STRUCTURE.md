# 3D Printer PINN-Seq3D Framework

## 项目概述

这是一个基于物理信息神经网络（PINN）和序列模型的3D打印质量预测与轨迹优化系统。项目结合了物理约束和深度学习，实现两大核心功能：

1. **质量预测与早停系统**：通过传感器数据实时预测打印质量，实现早期故障检测
2. **轨迹优化与误差补偿**：针对快速转角等关键位置进行误差预测与补偿

## 核心创新

- 🎯 **物理信息引导**：将热力学、振动动力学等物理定律嵌入损失函数
- 🔄 **混合序列建模**：Transformer + BiLSTM 捕捉多尺度时序依赖
- 🎭 **多任务学习**：统一框架同时处理质量预测和轨迹优化
- 📦 **模块化设计**：清晰的代码结构，易于扩展和维护

## 项目结构

```
3d_printer_pinn4ieee/
├── README.md                    # 项目说明文档
├── PROJECT_STRUCTURE.md         # 本文件
├── requirements.txt             # 依赖包
├── config/                      # 配置文件
│   ├── __init__.py
│   ├── base_config.py          # 基础配置
│   └── model_config.py         # 模型配置
│
├── data/                        # 数据目录
│   ├── raw/                    # 原始数据
│   ├── processed/              # 预处理后的数据
│   └── scripts/                # 数据生成脚本
│       ├── generate_physics_data.py
│       └── generate_trajectory_data.py
│
├── models/                      # 模型定义
│   ├── __init__.py
│   ├── base_model.py           # 基础模型类
│   ├── encoders/               # 编码器模块
│   │   ├── __init__.py
│   │   ├── pinn_encoder.py    # PINN引导的编码器
│   │   └── transformer_encoder.py
│   ├── decoders/               # 解码器模块
│   │   ├── __init__.py
│   │   ├── quality_decoder.py # 质量预测解码器
│   │   └── trajectory_decoder.py # 轨迹校正解码器
│   ├── physics/                # 物理约束模块
│   │   ├── __init__.py
│   │   ├── thermal_physics.py # 热力学约束
│   │   ├── vibration_physics.py # 振动动力学约束
│   │   └── energy_physics.py  # 能量守恒约束
│   └── unified_model.py        # 统一模型
│
├── training/                    # 训练相关
│   ├── __init__.py
│   ├── trainer.py             # 统一训练器
│   ├── losses.py              # 损失函数
│   ├── optimizers.py          # 优化器配置
│   └── schedulers.py          # 学习率调度器
│
├── evaluation/                  # 评估模块
│   ├── __init__.py
│   ├── metrics.py             # 评估指标
│   └── visualizer.py          # 可视化工具
│
├── inference/                   # 推理模块
│   ├── __init__.py
│   └── predictor.py           # 推理引擎
│
├── utils/                       # 工具函数
│   ├── __init__.py
│   ├── data_utils.py          # 数据处理工具
│   ├── physics_utils.py       # 物理计算工具
│   └── logger.py              # 日志工具
│
├── experiments/                 # 实验脚本
│   ├── train_quality_model.py # 训练质量预测模型
│   ├── train_trajectory_model.py # 训练轨迹校正模型
│   └── train_unified_model.py # 训练统一模型
│
└── checkpoints/                 # 模型检查点
    ├── quality_predictor/
    ├── trajectory_corrector/
    └── unified_model/
```

## 模型架构

### 统一模型架构（Unified PINN-Seq3D）

```
输入：时序传感器数据 [batch, seq_len, features]
    ↓
┌───────────────────────────────────────────────┐
│           共享编码器 (Shared Encoder)          │
│  ┌─────────────────────────────────────────┐  │
│  │  PINN-Guided Transformer Encoder        │  │
│  │  - Positional Encoding                  │  │
│  │  - Multi-Head Self-Attention (8 heads)  │  │
│  │  - Physics-Informed Feed-Forward        │  │
│  │  - Layer Normalization + Residual       │  │
│  │  - 6 Layers, d_model=256                │  │
│  └─────────────────────────────────────────┘  │
└───────────────────────────────────────────────┘
    ↓
    ├─────────────────┬─────────────────┐
    ↓                 ↓                 ↓
┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│ 质量预测分支  │ │ 故障分类分支  │ │ 轨迹校正分支  │
│ Quality      │ │ Fault        │ │ Trajectory   │
│ Decoder      │ │ Classifier   │ │ Decoder      │
│              │ │              │ │              │
│ - MLP Head   │ │ - Linear+Soft│ │ - BiLSTM     │
│ - Regression │ │ - 4 Classes  │ │ - Attention  │
│              │ │              │ │ - Regression │
└──────────────┘ └──────────────┘ └──────────────┘
    ↓                 ↓                 ↓
 质量指标预测      故障类型分类      误差补偿向量
(RUL, 温度等)     (4类故障)        (dx, dy)
```

## 物理约束

### 1. 热力学约束
```
∂T/∂t = α∇²T + Q(x,t)
- T: 温度场
- α: 热扩散系数
- Q: 热源项
```

### 2. 振动动力学约束
```
m·d²x/dt² + c·dx/dt + k·x = F(t)
- m: 质量
- c: 阻尼系数
- k: 刚度
- F: 外力
```

### 3. 能量守恒约束
```
dE/dt = P_in - P_out - P_loss
- E: 系统总能量
- P_in: 输入功率
- P_out: 输出功率
- P_loss: 损耗功率
```

### 4. 电机耦合约束
```
I_motor ∝ acceleration + vibration_load
- I_motor: 电机电流
- acceleration: 加速度
- vibration_load: 振动负载
```

## 数据格式

### 输入特征（传感器数据）
```
- 温度：喷嘴温度、床温、环境温度
- 振动：X/Y/Z轴振动加速度
- 电流：X/Y/Z轴电机电流
- 压力：挤出机压力
- 位置：X/Y/Z坐标
- 速度：X/Y/Z方向速度
- 时间戳
```

### 输出（质量预测）
```
- RUL（剩余使用寿命）
- 温度场重建
- 振动场重建
- 故障概率（4类）
- 质量评分
```

### 输出（轨迹校正）
```
- X方向位移补偿
- Y方向位移补偿
- Z方向位移补偿
- 速度调整建议
```

## 训练策略

### 多任务损失函数
```python
total_loss = λ_quality * L_quality +
             λ_fault * L_fault +
             λ_trajectory * L_trajectory +
             λ_physics * L_physics

其中：
- L_quality: 质量预测损失（MSE）
- L_fault: 故障分类损失（CrossEntropy）
- L_trajectory: 轨迹校正损失（MSE）
- L_physics: 物理约束损失（PDE残差）
```

### 优化器配置
- 优化器：AdamW
- 初始学习率：1e-4
- 权重衰减：1e-5
- 梯度裁剪：max_norm=1.0
- 学习率调度：Cosine Annealing + Warmup

### 训练技巧
- 混合精度训练（AMP）
- 梯度累积（accumulation_steps=2）
- Early Stopping
- 模型检查点保存

## 使用方法

### 1. 环境配置
```bash
pip install -r requirements.txt
```

### 2. 数据准备
```bash
python data/scripts/generate_physics_data.py
python data/scripts/generate_trajectory_data.py
```

### 3. 训练模型
```bash
# 训练统一模型
python experiments/train_unified_model.py

# 或分别训练
python experiments/train_quality_model.py
python experiments/train_trajectory_model.py
```

### 4. 推理预测
```python
from inference.predictor import UnifiedPredictor

predictor = UnifiedPredictor.load_from_checkpoint('checkpoints/unified_model/best_model.pth')
results = predictor.predict(sensor_data)

quality_metrics = results['quality']
fault_prediction = results['fault']
trajectory_correction = results['trajectory']
```

## 性能指标

### 质量预测模块
- 温度预测 RMSE: < 0.5°C
- 振动预测 RMSE: < 0.02mm
- RUL预测 RMSE: < 50s
- 故障分类准确率: > 95%

### 轨迹校正模块
- 转角误差减少: > 90%
- 平均预测误差: < 5mm
- 实时推理速度: > 100Hz

## 技术栈

- **深度学习框架**: PyTorch 2.0+
- **数值计算**: NumPy, SciPy
- **数据处理**: Pandas
- **可视化**: Matplotlib, Seaborn
- **日志**: TensorBoard, Weights & Biases

## 引用

如果本项目对你有帮助，请引用：

```bibtex
@misc{pinn_seq3d_2024,
  title={PINN-Seq3D: A Unified Framework for 3D Printing Quality Prediction and Trajectory Optimization},
  author={Your Name},
  year={2024},
  howpublished={\url{https://github.com/yourusername/3d_printer_pinn4ieee}}
}
```

## 许可证

MIT License

## 联系方式

- Email: your.email@example.com
- GitHub Issues: https://github.com/yourusername/3d_printer_pinn4ieee/issues
