"""
Evaluate implicit state inference model with visualizations.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate implicit state model')
    ...
    import pandas as pd
    pd.DataFrame(metrics).to_csv(results_dir / 'implicit_state_metrics.csv', index=False)
    ...
    axes[-1].axis('off')
    plt.tight_layout()
    plt.savefig(figures_dir / 'implicit_state_pred_vs_target.png', dpi=200)
    plt.close()
    ...
    axes[-1].axis('off')
    plt.tight_layout()
    plt.savefig(figures_dir / 'implicit_state_error_hist.png', dpi=200)
    plt.close()

    print(f'Metrics saved to: {results_dir / "implicit_state_metrics.csv"}')
