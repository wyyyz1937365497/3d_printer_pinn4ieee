% COLLECT_BOAT_ALL - 收集simple_boat5采样层数
%
% 处理simple_boat5_PLA_4h4m.gcode的采样层（间隔5层，共74层）
%
...
output_dir = 'data_simulation_simple_boat5_PLA_4h4m_sampled_74layers';
...
% 采样配置（间隔5层）
sampling_interval = 5;
layers_to_process = 1:sampling_interval:max_layer;
fprintf('采样间隔: %d\n', sampling_interval);
fprintf('采样层数: %d\n', length(layers_to_process));
fprintf('待处理层: %s\n', mat2str(layers_to_process));
fprintf('预计时间: %.1f 分钟 (GPU)\n\n', length(layers_to_process) * 0.5);
...
fprintf('预计需要3-8分钟，但会大幅加快后续仿真速度\n\n');
...
%% Helper function: Detect maximum layer number
